 ...
 * Register User page manages. Paginated and searchable by name/email.
 */
export const fetchTaggableUsers = async ({ skip = 0, limit = 20, search = '' } = {}) => {
  ...
  const res = await axios.post(
    `${Api_url}/api/v1/authorizedUsers/fetch?${params.toString()}`,
    {},
    { headers: jsonHeaders() }
  );
...
export const tagVehicleToUser = async ({ userId, vehicleNumber }) => {
  const res = await axios.patch(
    `${Api_url}/api/v1/authorizedUsers/tag-vehicle`,
...
export const untagVehicleFromUser = async ({ userId, vehicleNumber }) => {
  const res = await axios.patch(
    `${Api_url}/api/v1/authorizedUsers/untag-vehicle`,
    { userId, vehicleNumber },
    { headers: jsonHeaders() }
  );
  return res?.data?.body;
...
export const fetchRegisteredUser = async (userId) => {
  const res = await axios.post(
    `${Api_url}/api/v1/authorizedUsers/fetch?userId=${encodeURIComponent(userId)}&skip=0&limit=1`,
    {},
    { headers: jsonHeaders() }
  );
