...
const CameraNine = ({ cameraData = [],selectedVideo }) => {
  // Only use the first 9 cameras for this grid
  const cameras = cameraData.slice(0, 9);
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-8  lg:grid-cols-4 md:grid-cols-3 gap-2 "
    >
      {cameras.map((camera, idx) => (
        <div
          key={idx}
          className="relative border rounded overflow-hidden shadow-md aspect-video"
          onClick={() => {
