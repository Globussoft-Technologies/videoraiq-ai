...
const fetchUnauthorizedAccessLogs = async ({
  ...
  return axios.get(`${HOST}/api/v1/incidents/logs/unauthorized-access`, {
    params: {
      skip,
      limit,
      ...(startDate && { startDate }),
...
const REFRESH_KEY = 'unauthorized_access_auto_refresh_enabled';
const INTERVAL_KEY = 'unauthorized_access_auto_refresh_interval';
...
const UnauthorizedAccess = () => {
  ...
  const fetchAllForExport = async () => {
    const token = getAccessToken();
    const res = await axios.get(`${HOST}/api/v1/incidents/logs/unauthorized-access`, {
      params: {
        skip: 0,
        limit: 10000,
        ...(startDate && { startDate }),
  ...
  const exportToPDF = async () => {
    try {
      ...
      doc.setFontSize(12);
      doc.text('Unauthorized Access Logs', 14, 12);
      doc.setFontSize(9);
      ...
      doc.save('unauthorized_access_logs.pdf');
    } catch {
  ...
  const exportToExcel = async () => {
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, 'Unauthorized Access Logs');
      XLSX.writeFile(workbook, 'unauthorized_access_logs.xlsx');
    } catch {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      const res = await fetchUnauthorizedAccessLogs({
        skip,
        limit,
        startDate,
        endDate,
      ...
      console.log('Error fetching unauthorized access logs:', err);
      setError(err);
    } finally {
      setLoading(false);
    }
  }, [skip, limit, startDate, endDate, sortField, sortOrder, nvrIds, channelIds, severity, searchInput]);
        ...
        cell: ({ row }) => {
          ...
          return (
            <span className={styles.text}>
              {t
                ? moment.utc(t).tz(moment.tz.guess()).format('DD/MM/YYYY hh:mm A')
                : '--'}
            </span>
          );
        },
