...
import { X, Image, LayoutGrid, List, Filter, RotateCcw, Search, ChevronDown } from 'lucide-react';
...
const fetchWaterSpillageLogs = async ({
  skip,
  limit,
  startDate,
  endDate,
  ...
  return axios.get(`${HOST}/api/v1/incidents/logs/water-spillage-detection`, {
    params: {
      skip,
      limit,
      ...(startDate && { startDate }),
...
const REFRESH_KEY = 'water_spillage_auto_refresh_enabled';
const INTERVAL_KEY = 'water_spillage_auto_refresh_interval';

const WaterSpillageLogs = () => {
  ...
  const [reportStatus, _setReportStatus] = useState('');
  ...
  const fetchAllForExport = async () => {
    const token = getAccessToken();
    const res = await axios.get(`${HOST}/api/v1/incidents/logs/water-spillage-detection`, {
      params: {
        skip: 0,
        limit: 10000,
        ...(startDate && { startDate }),
  ...
  const exportToPDF = async () => {
    try {
      ...
      doc.setFontSize(12);
      doc.text('Water Spillage Detection Logs', 14, 12);
      doc.setFontSize(9);
      doc.text(`Generated on: ${moment().format('DD/MM/YYYY HH:mm')}`, 14, 18);

      const headers = ['#', 'Incident Name', 'NVR Name', 'Camera Name', 'Created At', 'Severity', 'Image'];
      ...
      doc.save('water_spillage_logs.pdf');
    } catch {
  ...
  const exportToExcel = async () => {
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, 'Water Spillage Logs');
      XLSX.writeFile(workbook, 'water_spillage_logs.xlsx');
    } catch {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [nvrIds, channelIds, severity, resolved, reportStatus]);
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      const res = await fetchWaterSpillageLogs({
        skip,
        limit,
        startDate,
        endDate,
      ...
      console.log('Error fetching water spillage logs:', err);
      setError(err);
    } finally {
      setLoading(false);
    }
  }, [skip, limit, startDate, endDate, sortField, sortOrder, nvrIds, channelIds, severity, resolved, reportStatus, searchInput]);
...
export default WaterSpillageLogs;
