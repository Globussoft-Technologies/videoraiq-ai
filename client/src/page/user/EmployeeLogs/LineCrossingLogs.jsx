...
import { Image, X, LayoutGrid, List, Filter, RotateCcw, Search, ChevronDown } from 'lucide-react';
...
const REFRESH_KEY = 'line_crossing_auto_refresh_enabled';
const INTERVAL_KEY = 'line_crossing_auto_refresh_interval';

const fetchLineCrossingLogs = async ({
  skip,
  limit,
  startDate,
  endDate,
  ...
  return axios.get(`${HOST}/api/v1/incidents/logs/line-crossing`, {
    params: {
      skip,
      limit,
      ...(startDate && { startDate }),
...
const LineCrossingLogs = () => {
  ...
  const fetchAllForExport = async () => {
    const token = getAccessToken();
    const res = await axios.get(`${HOST}/api/v1/incidents/logs/line-crossing`, {
      params: {
        skip: 0,
        limit: 10000,
        ...(startDate && { startDate }),
  ...
  const exportToPDF = async () => {
    try {
      ...
      doc.setFontSize(12);
      doc.text('Line Crossing Logs', 14, 12);
      doc.setFontSize(9);
      ...
      const tableRows = allLogs.map((row, i) => [i + 1, row.incidentName, row.nvrName, row.channelName, row.createdAt, row.severity, '']);
      ...
      doc.save('line_crossing_logs.pdf');
    } catch {
  ...
  const exportToExcel = async () => {
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, 'Line Crossing Logs');
      XLSX.writeFile(workbook, 'line_crossing_logs.xlsx');
    } catch {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetchLineCrossingLogs({
        skip,
        limit,
        startDate,
        endDate,
      ...
      console.log('Error fetching line crossing logs:', err);
      setError(err);
    } finally {
      setLoading(false);
    }
