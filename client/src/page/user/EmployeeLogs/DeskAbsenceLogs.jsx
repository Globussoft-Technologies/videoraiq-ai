...
import { ChartColumnBig, ChevronLeft, ChevronRight, ChevronDown, Filter, RotateCcw } from 'lucide-react';
...
const fetchDeskAbsenceLogs = async ({
  skip,
  limit,
  startDate,
  endDate,
...
const fetchZoneNames = async () => {
  const token = getAccessToken();
  return axios.get(
    `${HOST}/api/v1/incidents/logs/desk-absence/filter/zone-names`,
    {
      headers: {
        Accept: 'application/json',
        'x-access-token': token,
...
const DeskAbsenceLogs = () => {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetchDeskAbsenceLogs({
        skip,
        limit: LIMIT,
        startDate,
        endDate,
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetchZoneNames();
