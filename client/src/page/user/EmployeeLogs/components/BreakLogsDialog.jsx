...
import { getAttendanceUserLogs } from '../Api/get';
...
const resolveRowMoment = (log) => {
  if (!log) return null;
  const candidates = [log.login, log.logout, log.date];
...
const BreakLogsDialog = ({
  ...
  const buildFileBaseName = () => {
    ...
    const dateForFile = sanitizeFilename(
      formatRowDate(log) || moment().format('DD-MM-YYYY')
    );
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-white rounded-[12px] p-4 sm:p-6 w-full max-w-[95vw] sm:max-w-[640px] max-h-[85vh] overflow-y-auto top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
        <DialogHeader>
          <div className="flex items-center justify-between gap-3 pr-8">
            <DialogTitle className="text-[#333333] text-base font-semibold flex items-center gap-2">
              <Hourglass className="w-4 h-4 text-[#07486A]" />
              Break Logs
            </DialogTitle>
            {!loading && canEdit && breakEntries.length > 0 && (
              <button
                type="button"
                onClick={exportToPDF}
                className="flex items-center gap-1.5 bg-[#07486A] hover:bg-[#053a56] text-white rounded-[8px] px-3 py-1.5 text-xs font-medium cursor-pointer transition-colors"
                title="Export to PDF"
              >
        ...
        {/* Employee context */}
        {log && (
          <div className="flex items-center gap-3 mt-3 pb-3 border-b border-[#EEEEEE]">
            <div className="w-12 h-12 rounded-full overflow-hidden bg-white border border-[#E6E6E6] p-[1px] flex items-center justify-center">
              <img
                src={log.image}
                alt={log.name}
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[#696969] text-xs">{formatRowDate(log)}</span>
            </div>
          </div>
        )}
        ...
        {loading ? (
          <div className="mt-6 py-10 flex flex-col items-center text-center">
            <span className="text-sm text-[#696969]">Loading...</span>
          </div>
        ) : breakEntries.length === 0 ? (
          <div className="mt-6 py-10 flex flex-col items-center text-center">
            <div className="w-14 h-14 rounded-full bg-[#F3F4F6] flex items-center justify-center mb-3">
              <Hourglass className="w-7 h-7 text-[#9CA3AF]" />
            </div>
            <span className="text-sm text-[#696969]">No breaks recorded</span>
          </div>
        ) : (
          <>
            <div className="mt-4 flex flex-col gap-2.5 max-h-[45vh] overflow-y-auto pr-1 customscrollbar">
              {breakEntries.map((b, idx) => (
                <div
                  key={idx}
                  className="bg-[#FAFAFA] hover:bg-[#F2F4F7] transition-colors rounded-xl border border-[#EEEEEE] px-4 py-3"
                >
                  <div className="flex items-center justify-between gap-4 flex-wrap">
                    {/* Number + label */}
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full bg-[#07486A] text-white text-xs font-semibold flex items-center justify-center">
                        {idx + 1}
                      </div>
                      <span className="text-xs text-[#696969] font-medium uppercase tracking-wide">
                        Break {idx + 1}
                      </span>
                    </div>

                    {/* Duration pill */}
                    <span className="bg-[#07486A]/10 text-[#07486A] text-xs font-semibold px-3 py-1 rounded-full">
                      {formatBreakDuration(b.checkoutTime, b.checkinTime)}
                    </span>
                  </div>
                  ...
                  <div className="mt-3 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      <div className="w-8 h-8 rounded-full bg-white border border-[#E6E6E6] flex items-center justify-center flex-shrink-0">
                        <LogOut className="w-4 h-4 text-[#07486A]" />
                      </div>
                      <div className="flex flex-col min-w-0">
                        <span className="text-[10px] text-[#9CA3AF] uppercase tracking-wide">
                          Check out
                        </span>
                        <span className="text-sm text-[#333333] font-medium truncate">
                          {formatTime(b.checkoutTime, region)}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 flex-1 min-w-0 justify-end">
                      ...
                      <div className="w-8 h-8 rounded-full bg-white border border-[#E6E6E6] flex items-center justify-center flex-shrink-0">
                        <LogIn className="w-4 h-4 text-[#07486A]" />
                      </div>
                    </div>
                  </div>
                </div>
