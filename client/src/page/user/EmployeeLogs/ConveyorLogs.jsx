...
const formatConveyorStatus = (status) => {
  if (status === 'ON') return 'Loaded';
  if (status === 'OFF') return 'Not-Loaded';
  return status || '--';
};

const REFRESH_KEY = 'conveyor_auto_refresh_enabled';
const INTERVAL_KEY = 'conveyor_auto_refresh_interval';

const styles = {
  text: 'text-[#333333] text-xs font-normal',
};

const fetchConveyorLogs = async ({
  skip, limit, startDate, endDate, sortField, sortOrder,
  nvrId, nvrIds, channelId, channelIds,
  severity, resolved, reportStatus, status, search,
}) => {
  const token = getAccessToken();
  return axios.get(`${HOST}/api/v1/incidents/logs/conveyor-detection`, {
    params: {
      skip,
      limit,
      ...(startDate && { startDate }),
...
const ConveyorLogs = () => {
  ...
  const fetchAllForExport = async () => {
    const token = getAccessToken();
    const res = await axios.get(`${HOST}/api/v1/incidents/logs/conveyor-detection`, {
      params: {
        skip: 0,
        limit: 10000,
        ...(startDate && { startDate }),
    ...
    return list.map((item) => ({
      ...
      currentStatus: formatConveyorStatus(item.currentStatus),
  ...
  const exportToPDF = async () => {
    try {
      ...
      doc.setFontSize(12);
      doc.text('Conveyor Detection Logs', 14, 12);
      doc.setFontSize(9);
  ...
  const exportToExcel = async () => {
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, 'Conveyor Logs');
      XLSX.writeFile(workbook, 'conveyor_logs.xlsx');
    } catch {
  ...
  // Fetch channels whenever selected NVRs change
  useEffect(() => {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [startDate, endDate, selectedNvrIds, selectedChannelIds, severity, resolved, reportStatus, status]);
  ...
  const fetchLogs = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetchConveyorLogs({
        skip, limit, startDate, endDate, sortField, sortOrder,
        nvrIds: selectedNvrIds,
        channelIds: selectedChannelIds,
        severity,
        resolved: resolved !== '' ? resolved : undefined,
        reportStatus: reportStatus !== '' ? reportStatus : undefined,
        status,
        search: searchInput,
      });
      ...
      console.log('Error fetching conveyor logs:', err);
      setError(err);
    } finally {
      setLoading(false);
    }
  }, [skip, limit, startDate, endDate, sortField, sortOrder,
      selectedNvrIds, selectedChannelIds, severity, resolved, reportStatus, status, searchInput]);
    ...
    () => [
      {
        ...
        header: () => (
          <button onClick={() => handleSort('incidentName')} className="flex items-center gap-1 cursor-pointer">
            Incident Name 
        ...
        cell: ({ row }) => {
          ...
          return (
            <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${
              ...
              {isEmpty ? '--' : formatConveyorStatus(s)}
            </span>
          );
        },
      },
      ...
      },
      {
        accessorKey: 'nvrName',
        header: () => (
          <button onClick={() => handleSort('nvrData.nvrName')} className="flex items-center gap-1 cursor-pointer">
            NVR Name
      ...
      },
      {
        ...
        header: () => (
          <button onClick={() => handleSort('createdAt')} className="flex items-center gap-1 cursor-pointer">
           Time of Incident
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            <Select value={status} onValueChange={setStatus}>
              <SelectTrigger className="w-full h-9 border border-[#C7C7C7] rounded-lg cursor-pointer text-sm bg-white text-[#595959]">
  ...
  {row.currentStatus ? (
    <span className="text-xs font-medium px-2.5 py-1 rounded-full text-[#07486A] bg-[#EEF6FB]">
      {formatConveyorStatus(row.currentStatus)}
    </span>
  ) : (
