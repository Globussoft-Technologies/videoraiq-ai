  ...
  try {
    ...
    try {
      ...
      if (emailAddresses?.length && alertAdminContext?.emailAlertsEnabled !== false) {
        ...
        } else if (detectionType === "vehicleCheckInOut") {
          let mailResponse = await MailResponse.vehicleCheckInOut(emailAddresses, incidentData, detectionType, nvrData, channelData, adminTz)
