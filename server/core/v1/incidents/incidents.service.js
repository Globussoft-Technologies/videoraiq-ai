...
class IncidentsService {
      ...
      if (
        ...
        if (recentIncident) {
          ...
          if (
            ...
            recentIncident.timeSeries.push({
              timestamp: req?.body?.timeOfIncident,
              type,
              entry,
              exit,
            });
  ...
  async getAllIncidents(req, res, next) {
    try {
      ...
      // countDocuments can't carry the tagging pipeline stages, so the
      // filtered case counts through an aggregation instead. Everything else
      // keeps the cheaper count it always used.
      const totalCount = vehicleStages.length
        ? (
  ...
  async _fetchIncidentLogs({
    ...
    if (escapedSearch && postLookupSearch) {
      ...
      basePipeline.push(
        ...
        }, // strip helper — response shape unchanged
      );
    }
