...
class IncidentsValidator {
  ...
  editIncidentDetails(body) {
    const schema = Joi.object().keys({
      ...
      totalEntry: Joi.number().optional().min(0),
      totalExit: Joi.number().optional().min(0),
      type: Joi.string().optional(),
      entry: Joi.number().optional().min(0),
      exit: Joi.number().optional().min(0),
      count_mode: Joi.string().optional(),
      eventCount: Joi.number().optional().min(0),
