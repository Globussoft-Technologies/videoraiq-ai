...
const fetchVehicleObstructionLogs = async ({
  skip,
  limit,
  startDate,
  endDate,
...
const REFRESH_KEY = 'vehicle_obstruction_auto_refresh_enabled';
const INTERVAL_KEY = 'vehicle_obstruction_auto_refresh_interval';
...
const ANPRLogs = () => {
  ...
  const [vehicleNumberOpen, setVehicleNumberOpen] = useState(false);
  ...
  // (ANPRLogs, accessLogs, attendanceLogs, global, ...). Resolve in order:
  ...
  // ANPRLogs.view), so if the user lacks the specific permission for
  ...
  const fetchAllForExport = async () => {
    const token = getAccessToken();
    const res = await axios.get(`${HOST}/api/v1/incidents/logs/vehicle-detection`, {
      params: {
        skip: 0,
        limit: 10000,
        ...(startDate && { startDate }),
    ...
    const handle = setTimeout(async () => {
      try {
        ...
        const res = await axios.get(
          `${HOST}/api/v1/incidents/logs/vehicle-detection/numbers`,
          {
            params: {
              ...(vehicleNumberSearch && { search: vehicleNumberSearch }),
            },
            headers: {
              Accept: 'application/json',
              'x-access-token': token,
  ...
  // Fetch channels when nvrIds change
  useEffect(() => {
  ...
  const VehicleNumberSelect = (
    <Popover open={vehicleNumberOpen} onOpenChange={setVehicleNumberOpen}>
      <PopoverTrigger asChild>
        <button
          type="button"
          className="h-9 md:h-10 min-w-[180px] px-3 border border-[#C7C7C7] rounded-lg text-sm bg-white text-[#595959] cursor-pointer flex items-center justify-between gap-2"
        >
          <span className="truncate">
            {vehicleNumber || 'Vehicle Number'}
          </span>
          <ChevronDown className="w-4 h-4 text-[#5D5D5D] shrink-0" />
        </button>
      </PopoverTrigger>
      <PopoverContent className="w-[240px] p-0 rounded-lg" align="start">
        <div className="p-2 border-b border-gray-100">
          <div className="relative">
            ...
            <Input
              ...
              className="h-8 pl-8 pr-2 text-xs border border-[#D1D5DB] shadow-none"
            />
          </div>
        </div>
        <div className="max-h-60 overflow-y-auto py-1">
          <button
            ...
            onClick={() => {
              setVehicleNumber('');
              setVehicleNumberOpen(false);
            }}
            className={`w-full text-left px-3 py-1.5 text-xs cursor-pointer hover:bg-gray-100 ${
              !vehicleNumber ? 'bg-gray-50 font-medium text-[#07486A]' : 'text-[#333333]'
            }`}
          >
            All Vehicles
          </button>
          {vehicleNumberList.length === 0 ? (
            <div className="px-3 py-2 text-xs text-gray-400">No vehicle numbers</div>
          ) : (
            vehicleNumberList.map((vn) => (
              <button
                ...
                className={`w-full text-left px-3 py-1.5 text-xs cursor-pointer hover:bg-gray-100 ${
                  vehicleNumber === vn ? 'bg-gray-50 font-medium text-[#07486A]' : 'text-[#333333]'
                }`}
              >
                {vn}
              </button>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            <MultiSelect
              options={nvrOptions}
              value={nvrIds}
              onChange={(value) => { setNvrIds(value); if (value.length === 0) setChannelIds([]); }}
    ...
    () => [
      {
        ...
        header: () => (
          <button
            onClick={() => {
              setSortField('incidentName');
              setSortOrder((prev) => (prev === 'asc' ? 'desc' : 'asc'));
            }}
            className="flex items-center gap-1 cursor-pointer"
          >
            Incident Name
            {/* {sortField === 'incidentName' ? (
              sortOrder === 'asc' ? (
        ...
        cell: ({ row }) => (
          <span className={styles.text}>{row.original.incidentName}</span>
        ),
      },
      {
        accessorKey: 'nvrName',
        header: () => (
          <button
            onClick={() => {
              setSortField('nvrData.nvrName');
              setSortOrder((prev) => (prev === 'asc' ? 'desc' : 'asc'));
            }}
            className="flex items-center gap-1 cursor-pointer"
          >
            NVR Name
            {/* {sortField === 'nvrData.nvrName' ? (
              sortOrder === 'asc' ? (
                <ArrowUp className="w-4 h-4" />
              ) : (
                <ArrowDown className="w-4 h-4" />
        ...
        cell: ({ row }) => (
          <span className={styles.text}>{row.original.nvrName}</span>
        ),
      },
      {
        ...
        header: 'Camera Name',
        cell: ({ row }) => (
          <span className={styles.text}>{row.original.channelName}</span>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span className={styles.text}>{row.original.vehicleNumber}</span>
        ),
      },
      {
  ...
  // this, users briefly see AccessDenied while the redirect-to-accessible
  // sub-route effect is still resolving.
        ...
        >
          ...
          <div className="flex items-center border border-[#C7C7C7] rounded-[8px] overflow-hidden  h-9">
            <button
              ...
              className="flex cursor-pointer items-center justify-center px-2.5 h-full bg-[#07486A] text-white"
              title="List view"
            >
            ...
            <button
              ...
              className="flex cursor-pointer items-center justify-center px-2.5 h-full bg-white text-[#595959] hover:bg-gray-50"
              title="Grid view"
            >
              ...
              <div className="flex flex-wrap items-center gap-3 pt-2">
                ...
                <div className="w-full md:flex md:items-center md:ml-auto md:w-auto gap-3 flex flex-wrap">
                  ...
                  <div className="flex items-center border border-[#C7C7C7] rounded-[8px] overflow-hidden h-9">
                    <button
                      onClick={() => setViewMode('list')}
                      className="flex items-center justify-center px-2.5 h-full bg-white text-[#595959] hover:bg-gray-50"
                      title="List view"
                    >
                      <List className="w-4 h-4" />
                    </button>
                    <button
                      onClick={() => setViewMode('grid')}
                      className="flex items-center justify-center px-2.5 h-full bg-[#07486A] text-white"
                      title="Grid view"
                    >
                      <LayoutGrid className="w-4 h-4" />
                    </button>
                  ...
                  {rows.map((row) => (
                    ...
                    >
                      ...
                      <div className="p-3 space-y-2">
                        ...
                        <div>
                          <p className="text-[10px] font-medium text-[#888] uppercase tracking-wide">Vehicle Number: 
                                 <span className="text-xs text-[#333333] truncate">{row.vehicleNumber}</span>
                          </p>
            ...
            {!loading && rows.length > 0 && (
              <div className="mt-6 grid grid-cols-3 items-center gap-4">
                <div className="flex items-center gap-3">
                  <div className="text-sm text-[#333333] bg-[#F5F5F5] px-3 py-1.5 font-normal rounded-[5px] inline-flex items-center gap-2">
                    Total logs -{' '}
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <select
                      ...
                      className="h-8 border border-[#C7C7C7] rounded-lg text-xs text-[#595959] bg-white px-2 cursor-pointer focus:outline-none focus:ring-1 focus:ring-[#07486A]"
                    >
                ...
                <div className="flex items-center justify-center gap-2">
                  ...
                  {Array.from({ length: Math.max(1, Math.ceil(totalCount / limit)) }, (_, i) => i + 1)
                    .filter((p) => {
                      const totalPgs = Math.ceil(totalCount / limit);
                      return p === 1 || p === totalPgs || Math.abs(p - currentPage) <= 1;
                    })
