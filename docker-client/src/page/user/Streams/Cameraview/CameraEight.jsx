...
const CameraEight = ({ cameraData = [],selectedVideo }) => {
  // Only use the first 8 cameras for this grid
  const cameras = cameraData.slice(0, 8);
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-2 [@media(min-width:1280px)_and_(max-width:1535px)]:h-[calc(100vh-102px)] ">
      {cameras.map((camera, idx) => (
        <div
          key={idx}
          className="relative border rounded overflow-hidden shadow-md 2xl:aspect-video [@media(max-width:1280px)]:aspect-video"
          onClick={() => {
