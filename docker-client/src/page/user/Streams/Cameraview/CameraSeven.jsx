...
const CameraSeven = ({ cameraData = [],selectedVideo }) => {
  // Only use the first 7 cameras for this grid
  const cameras = cameraData.slice(0, 7);
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 sm:grid-cols-2 lg:grid-cols-5 gap-3  xl:h-[calc(100vh-102px)] ">
      {cameras.map((camera, idx) => (
        <div
          key={idx}
          className="relative border rounded overflow-hidden shadow-md [@media(max-width:1280px)]:aspect-video "
          onClick={() => {
