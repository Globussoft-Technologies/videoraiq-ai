...
const IncidentCard = ({ item, onClick, resolved, onMarkResolved, canEdit, onReport, deleteMode, selectedForDelete, onToggleDelete }) => {

  return (
    <div
      className={`rounded-[8px] md:rounded-[10px] 2xl:rounded-[14px] overflow-hidden bg-white shadow border transition-shadow ${deleteMode && selectedForDelete ? 'border-[#CE241C] shadow-md' : 'border-[#E4E4E4] hover:shadow-lg'} cursor-pointer`}
      onClick={onClick}
    >
      {/* Video + overlays */}
      <div className="relative">
        ...
        {/* Top left area - Zone, Mark as Resolved, or Delete checkbox */}
        <div className="absolute top-1.5 md:top-2 2xl:top-2.5 left-1.5 md:left-1.5 2xl:left-2 flex items-center gap-2">
          ...
          {deleteMode ? (
            <div
              className="flex items-center bg-black/80 hover:bg-black rounded-full 2xl:py-1.5 2xl:px-3 py-1 px-2 space-x-2 transition-colors duration-200 cursor-pointer"
            ...
            >
              <Checkbox
                ...
                className="border size-3 md:size-3 2xl:size-4 data-[state=checked]:bg-[#CE241C] data-[state=checked]:text-white border-[#8D8D8D] cursor-pointer"
              />
              <label
        ...
        {/* Top right - Timestamp */}
        <div className="absolute bg-black/80 top-1 md:top-1.5 2xl:top-2 right-8 text-[8px] md:text-[10px] 2xl:text-[12px] font-medium text-[#FFFFFF] px-2 md:px-2.5 2xl:px-3 py-1 md:py-1.5 2xl:py-[5px] rounded-full">
          {item?.createdAt
