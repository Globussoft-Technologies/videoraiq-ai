...
export const getChannels = async ({ skip = 0, limit = 100, nvrId = '', search = '', camType = '', engines = '' } = {}) => {
  const token = getAccessToken();
  ...
  if (engines) params.set('engines', Array.isArray(engines) ? engines.filter(Boolean).join(',') : engines);
